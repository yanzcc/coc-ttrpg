...
- **技能检定增强**
  - **智能技能名解析**（`resolve_skill`）：LLM 可能写出白名单外的技能名（如「灵感」「聆听或观察」），系统自动映射到正确技能或属性（灵感→INT、聆听或观察→聆听），支持别名、括号规范化、专攻家族回退，绝不 fallback 到 value=1
  - **默认普通难度**：未显式指定时一律普通，避免过度使用困难/极难
  - **每行动单检定**：一次玩家行动最多触发一次技能检定，防止叠加
  - **玩家显式技能尊重**：玩家文本中写「使用 X 技能」时强制触发对应检定
  - **失败不泄露线索**：检定失败后续写会注入明确指令，禁止描述发现任何有用信息
- **NPC 路由防错标**：场景切换时不再将旧 NPC 的对话延续到新场景；提到不同 NPC 名字时自动中断接话
- **场景旁白去重**：自动剥离场景守密人违规写出的 NPC 对白，防止与 NPC 守密人输出重复
